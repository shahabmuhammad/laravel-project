            ...
            @foreach ($bookmarks as $paper)
                <div class="col-md-4">
                    <div class="card shadow-sm p-3">
                        ...
                        <p class="mb-1"><strong>Author:</strong> {{ $paper->user->name ?? 'N/A' }}</p>
                        <p class="mb-1"><strong>University:</strong> {{ $paper->publisher->name ?? 'N/A' }}</p>
                        <p class="mb-1"><strong>Category:</strong>
                            @if ($paper->categories && $paper->categories->count() > 0)
                                {{ $paper->categories->pluck('name')->implode(', ') }}
                            @else
                                N/A
                            @endif
                        </p>
                        <div class="d-flex justify-content-between mt-2">
                            <a href="{{ route('admin.researches.show', encrypt($paper->id)) }}"
